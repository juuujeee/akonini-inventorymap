import inventoryMap from "./script_inventory_map.js";
...
export default function home() {
    ...
    function loadProjectView() {
        ...
        let tblParser = projectListContainerView();

        tblParser.querySelector('.jsCloneProject').addEventListener('click', async function (e) {

            let parentEl = e.currentTarget.closest('.jsClone');

            let projectID = parentEl.getAttribute('data-id');
            let projectName = parentEl.querySelector('.jsProjectName').textContent;
            ...
            let url = AppGlobal.baseUrl + 'inventory-map';

            history.pushState({}, null, url);

            inventoryMap(config).init();
        });
    ...
    function projectListContainerView() {
        ...
        let view = `
                <div class="projectlist-tr jsClone">
                    <div class="project-tr1 jsCloneProject">
        ...
        return new DOMParser().parseFromString(view, 'text/html').querySelector('.jsClone');
